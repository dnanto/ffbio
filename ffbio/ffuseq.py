...
def main(argv):
    ...
    with args.file as handle:
        ...
        records = OrderedDict(
            (key, list(val))
            for key, val in groupby(sorted(SeqIO.parse(handle, args.fmt), key=keyfunc), keyfunc)
        )

    # map unique sequence index to unique hash
    if args.map:
        with args.map.open("w") as handle:
            width = len(str(len(records)))
            print("idx", "key", "id", "description", "length", sep="\t", file=handle)
            for idx, ele in enumerate(records.items(), start=1):
                idx, key, val = f"{idx:0{width}d}", ele[0], ele[1]
                for rec in val:
                    print(idx, key, rec.id, rec.description, len(rec), sep="\t", file=handle)
                    rec.id = idx
